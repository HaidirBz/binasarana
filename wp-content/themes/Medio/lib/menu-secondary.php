<?php
    ...
    function themater_menu_secondary_display()
    {
        global $flexitheme;
        ?>
			<?php wp_nav_menu( 'depth=' . $flexitheme->get_option('menu_secondary_depth') . '&theme_location=' . $flexitheme->options['menus']['menu-secondary']['theme_location'] . '&container_class=' . $flexitheme->options['menus']['menu-secondary']['wrap_class'] . '&menu_class=' . $flexitheme->options['menus']['menu-secondary']['menu_class'] . '&fallback_cb=' . $flexitheme->options['menus']['menu-secondary']['fallback'] . ''); ?>
              <!--.secondary menu--> 	
        <?php
    }
    ...
    function themater_menu_secondary_default()
    {
        global $flexitheme;
        ?>
        <div class="<?php echo $flexitheme->options['menus']['menu-secondary']['wrap_class']; ?>">
			<ul class="<?php echo $flexitheme->options['menus']['menu-secondary']['menu_class']; ?>">
				<?php wp_list_categories('depth=' .  $flexitheme->get_option('menu_secondary_depth') . '&hide_empty=0&orderby=name&show_count=0&use_desc_for_title=1&title_li='); ?>
			</ul>
		</div>
        <?php
    }
    ...
    function themater_menu_secondary_js()
    {
        ...
        $return = '';
            ...
            $menu_secondary_arrows = $flexitheme->display('menu_secondary_arrows') ? 'true' : 'false';
            $menu_secondary_shadows = $flexitheme->display('menu_secondary_shadows') ? 'true' : 'false';
            $menu_secondary_delay = $flexitheme->display('menu_secondary_delay') ? $flexitheme->get_option('menu_secondary_delay') : '800';
            $menu_secondary_speed = $flexitheme->display('menu_secondary_speed') ? $flexitheme->get_option('menu_secondary_speed') : '200';
            
            switch ($flexitheme->get_option('menu_secondary_effect')) {
                case 'standart' :
            ...
            $return .= "jQuery(function(){ \n\tjQuery('ul." . $flexitheme->options['menus']['menu-secondary']['superfish_class'] . "').superfish({ \n\t";
            ...
            $return .= "jQuery('.menu-secondary-container').mobileMenu({
                defaultText: '" . $flexitheme->get_option('menu_secondary_mobile_title') . "',
