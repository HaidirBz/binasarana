...
class ThematerAdmin
{
    ...
    {
        global $flexitheme, $pagenow;
        $this->theme = $flexitheme;
            ...
            <h2>Theme Options | 
                <span class="tt-themename">
                    <?php 
                        ...
                        $flexitheme_version =  $themater_current_theme->{'Version'};
                        $flexitheme_url =  $themater_current_theme->{'ThemeURI'};
                        if($flexitheme_url) {
                            printf("%s" . $this->theme->theme_name . " $flexitheme_version%s", "<a href=\"$flexitheme_url\" target=\"_blank\" title=\"Browse to Theme Homepage\">", "</a>");
                        } else {
                            echo $this->theme->theme_name . " $flexitheme_version";
                        }
                    ?> 
                </span>
            </h2>
    ...
    function optionsPageMenu()
    {
        ?>
            ...
            <div class="tt-menu-wrap">
                ...
                <?php
                    $menuid = 0;
                    $flexitheme_optionspage = $this->do_priority($this->theme->admin_options);
                                            
                    if( is_array($flexitheme_optionspage) && count($flexitheme_optionspage) > 0) {
                        
                        foreach($flexitheme_optionspage as $optionspage) {
                            $menuid++;
